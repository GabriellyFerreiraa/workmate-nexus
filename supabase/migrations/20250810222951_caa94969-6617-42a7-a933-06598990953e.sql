...
-- 4) Policy: Analysts can request cancellation of their approved requests (use text casts to avoid enum new value transaction issue)
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    WITH CHECK (analyst_id = auth.uid() AND status::text = 'cancel_pending' AND cancel_reason IS NOT NULL);
  END IF;
END $$;

-- 5) Update delete policy to include canceled (use text casts)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies 
  ...
  USING (
    analyst_id = auth.uid() 
    AND status::text IN ('approved', 'rejected', 'canceled')
  );
END $$;
